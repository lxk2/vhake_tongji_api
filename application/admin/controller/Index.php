...
class Index extends Admin
{
    ...
    public function checkUpdate()
    {
        ...
        if ($result['code'] == 1) {
            return json([
                'update' => '<a class="badge badge-primary" href="http://www.vhake.com/download" target="_blank">有新版本：'.$result["version"].'</a>',
                'auth'   => $result['auth']
            ]);
        } else {
            return json([
                'update' => '',
//                'auth'   => $result['auth']
                'auth'   => '<a class="label label-success" href="#" target="_blank">已授权</a>'
