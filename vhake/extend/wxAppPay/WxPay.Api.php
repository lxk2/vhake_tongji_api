...
require_once "WxJsPay.Config.php";
...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if($inputObj->GetTrade_type() == 'JSAPI') {
            ...
            $inputObj->SetAppid($config_data['APPID']);//公众账号ID
            $inputObj->SetMch_id($config_data['MCHID']);//商户号
            $inputObj->SetNotify_url($config_data['NOTIFY_URL']);//异步通知url
        } else {
...
//        dump($inputObj);die;
        ...
        if($inputObj->GetTrade_type() == 'JSAPI') {
            ...
            // 统一下单接口返回正常的prepay_id，再按签名规范重新生成签名后，将数据传输给h5。
            // 参与签名的字段名为appId，partnerId，prepayId，nonceStr，timeStamp，package。注意：package的值格式为Sign=WXPay
            $time_stamp = time();
            //输出参数列表
            $prePayParams =array();
            $prePayParams['appId']		= $result['appid'];
            $prePayParams['timeStamp']	= (string)$time_stamp;
            ...
            $paySign = '';
            ksort($prePayParams);
            foreach($prePayParams as $key=>$val) {
                if(empty($val)) continue;
                $paySign .= $key . '='. $val. '&';
            }
            $paySign .= 'key='. $config_data['KEY'];
            $paySign = strtoupper(md5($paySign));
            $prePayParams['paySign'] = $paySign;
