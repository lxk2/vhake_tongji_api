...
class QiNiu extends Plugin
{
    ...
    public $info = [
        ...
        // 插件描述[选填]
        'description' => '仅支持VhakePhp1.0.6以上版本，安装后，需将【<a href="/admin.php/admin/system/index/group/upload.html">上传驱动</a>】将其设置为“七牛云”。在附件管理中删除文件，并不会同时删除七牛云上的文件。',
        // 插件作者[必填]
        'author'      => 'Vhake',
        // 作者主页[选填]
    ...
    public function install(){
        if (!version_compare(config('dolphin.product_version'), '1.0.6', '>=')) {
            $this->error = '本插件仅支持VhakePhp1.0.6或以上版本';
            return false;
        }
        $upload_driver = Db::name('admin_config')->where(['name' => 'upload_driver', 'group' => 'upload'])->find();
        if (!$upload_driver) {
            $this->error = '未找到【上传驱动】配置，请确认VhakePhp版本是否为1.0.6以上';
            return false;
        }
